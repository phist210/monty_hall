...
def game_loop(wins_and_losses):
    ...
    doors = ['1', '2', '3']
    print("\t\tMonty (Python!) Hall Game Simulation\n")
    ...
    print("{} was chosen".format(door))
    print("These are the remaining doors:")
    print(playing_field)

    print("\nThe host reveals {}".format(pool))

    for door in playing_field:
        if door[1] != pick:  # if ==, 2/3 loss
            if door[0] == 'prize':
...
def main():
    ...
    wins_and_losses = []
    game_loop(wins_and_losses)
    while len(wins_and_losses) < 1000:
        game_loop(wins_and_losses)
